...
public class TipoDocumentoController {
	...
	@GetMapping("/{id}")
	public ResponseEntity mostarUno(@PathVariable int id){
		try{
			...
			documento = documentoRepository.findById(id).get();
			unDocumento.setId(documento.getId());
			unDocumento.setNombre(documento.getNombre());
			unDocumento.setDescripcion(documento.getDescripcion());
			
			return ResponseEntity.ok(unDocumento);
		}
		catch(Exception e){
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Problema: "+e);
		}
	...
	@DeleteMapping("/{id}")
	public ResponseEntity borrarRegistro(@PathVariable int id){
		try{
			...
			if(!documento.get().getPersonas().isEmpty()){
				throw new Exception("El documneto que se intenta eliminar esta ligado a Personas.");
			}
